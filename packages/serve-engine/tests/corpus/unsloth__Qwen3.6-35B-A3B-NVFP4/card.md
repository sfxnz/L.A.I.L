...
- qwen3_5
---
# Read our How to [Run Qwen3.6 NVFP4 Guide!](https://unsloth.ai/docs/models/qwen3.6#nvfp4)
<div>
...
* **1.56x faster** throughput than other NVFP4 quants.
...
* Use the [35B NVFP4 Fast](https://huggingface.co/unsloth/Qwen3.6-35B-A3B-NVFP4-Fast) version for **1.79x faster** at a little less accuracy

<img width="200px" src="https://qianwen-res.oss-accelerate.aliyuncs.com/Qwen3.6/logo.png">
...
Read all benchmarks in our [NVFP4 blog](https://unsloth.ai/docs/models/qwen3.6#nvfp4)
...
[vLLM](https://github.com/vllm-project/vllm) is the recommended serving path for this NVFP4 checkpoint. `vllm>=0.19.0` is recommended for Qwen3.6 NVFP4 artifacts:
```shell
uv pip install vllm --torch-backend=auto
```

The following command creates an OpenAI-compatible API endpoint at `http://localhost:8000/v1`:

```shell
vllm serve unsloth/Qwen3.6-35B-A3B-NVFP4 --trust-remote-code --dtype bfloat16 --max-model-len 4096
```

Increase `--max-model-len` only after checking available GPU memory. For long-context serving, follow the YaRN notes below and keep the model ID pointed at `unsloth/Qwen3.6-35B-A3B-NVFP4`.
